...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Change to ["http://127.0.0.1:8000"] in production
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# ✅ Add explicit OPTIONS route to fix preflight requests
...
# Load COCO labels (Ensure this file exists)

# Ensure correct model path
MODEL_PATH = "portfolio/model.tflite"
...
def process_image(image_bytes):
    ...
    for i in range(len(scores)):
        if scores[i] > 0.5:  # Confidence threshold
            ...
            cv2.putText(original_image, label, (int(x_min), int(max(y_min - 10, 20))),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
async def predict(file: UploadFile = File(...)):
    ...
    encoded_image = await loop.run_in_executor(None, process_image, image_bytes)
